...
public class healthbarAir : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (device.IsReading)
        {
            ...
            if (msg != null)
            {
                ...
                // double p_volt = volt;
                ppm = double.Parse(content); ; //convert content into double from string
                //volt = p_volt * 0.8 + volt * 0.2;

                statusText.text = "MSG : " + ppm.ToString();
                ...
                */
                newppm = ((1023 - ppm) / 1023) * 100;  //convert into health percentage

                //volt = ((3000 - ntu) / 3000) * 100;
                float health = (float)newppm;  //convert double to float
                                               //float health = (float)volt;
